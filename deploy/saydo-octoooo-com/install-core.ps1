# SayDo 一键安装核心脚本(Windows PowerShell 5.1+ / PowerShell 7)。本文件带 UTF-8 BOM,只由 install.ps1 引导脚本以 -File 方式运行:
#   irm https://saydo.octoooo.com/install.ps1 | iex      (install.ps1 是纯 ASCII、无 BOM 的引导,会下载本文件并以 -File 执行;PS 5.1 的 irm 会把 BOM 留在字符串里,直接 iex 本文件会解析失败)
...
if ($sysNode -and (Get-NodeMajor $sysNode.Source) -eq $NodeMajor) {
  ...
  if (-not $NodeExe) {
    Write-Info "本机没有 Node $NodeMajor,下载官方 Node $NodeMajor 到 $Toolchain(不改系统;约 30 MB)"
    # 先官方源,连不上自动改用镜像;SAYDO_INSTALL_MIRROR=1 直接用镜像。
    $nodeSrc = $NodeDist
    if ($env:SAYDO_INSTALL_MIRROR -eq "1") { $nodeSrc = $NodeDistMirror }
    else {
      try { Invoke-WebRequest -Uri "$NodeDist/index.json" -Method Head -TimeoutSec 15 -UseBasicParsing | Out-Null }
      catch { $nodeSrc = $NodeDistMirror; Write-Info "nodejs.org 连不上,改用 $NodeDistMirror" }
    }
    $index = Invoke-RestMethod -Uri "$nodeSrc/index.json" -TimeoutSec 60
    ...
    Invoke-WebRequest -Uri "$nodeSrc/$nodeVer/$nodePkg" -OutFile (Join-Path $tmp $nodePkg) -TimeoutSec 600
    Invoke-WebRequest -Uri "$nodeSrc/$nodeVer/SHASUMS256.txt" -OutFile (Join-Path $tmp "SHASUMS256.txt") -TimeoutSec 60
...
Write-Host ""
Write-Ok "安装完成。下一步:"
Write-Host "  1. 准备一个 AI 供给(二选一;第一次打开控制台时向导会检测):"
Write-Host "     - 本机 AI CLI 已安装并登录,例如 codex login / claude auth login / cursor-agent login"
Write-Host "     - 或一个 OpenAI 兼容 API key(OpenRouter / OpenAI / Anthropic / DeepSeek),在向导里填"
Write-Host "  2. 启动:saydo up(浏览器会打开 http://localhost:47100 的控制台;本终端若还找不到 saydo,直接运行:$launcher up)"
Write-Host "     不想自动开浏览器加 --no-open,之后用 saydo open;Ctrl+C 优雅停止。"
Write-Host "  该包只含 daemon 与 Web 控制台;语音 pipeline 与系统常驻不在其中;Windows 当前以前台方式运行(关闭终端即停)。"
