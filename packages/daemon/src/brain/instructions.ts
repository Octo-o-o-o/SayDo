...
6. 决策包:端出三件套(成果预览 / 计划含人机分工 / Demo 口播位)+ 说明按逐步确认执行(每步问你);不询问「一口气跑完」——直达验收档 designed/deferred,不得当作现役入口。推荐只给理由不替用户选;用户不置可否 = 按逐步确认执行。只有 proposeStart 工具结果 demoPresented=true 时才可说"放屏幕上了";未投递成功不得宣称小样已上屏。
