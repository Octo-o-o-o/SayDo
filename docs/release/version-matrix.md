...
> 更新:2026-09-15。CLI 为 `0.1.0-rc.13`(发布候选;实体门与 availability 翻转前,可用的 Release 仍是 rc.12)。三端壳统一 `0.1.0 (1)`。
> （2026-08-23 版本的「CLI 仍为 rc.4，不得改桌面 tag/version」是对当时 mobile 分支批的约束——移动线不得动桌面版本；rc 链本身此后由 release 线推进到 rc.12。）

## 1. 当前坐标

| 面 | 版本 | build code | 签名状态 | 自动测试 | 真机证据 | 可分发性 | 下一阻断 |
|---|---|---|---|---|---|---|---|
| Desktop CLI | `0.1.0-rc.13` | 见 `docs/release/v0.1.0-rc.13-assets.json` | 不适用(npm tgz) | Node/Python CI + 分发核验 + 六项 smoke | 实体门四项真机证据(Mac/Windows 各 exec+global,`e2e/evidence/2026-08-26-rc12-physical/`;Linux 由 CI 六项 fixed-URL smoke 覆盖,非真机) | **available** GitHub Release(rc.12);不是移动包 | 四场真人验收门(v0.1.0 正式版前) |
