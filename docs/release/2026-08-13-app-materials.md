...
| 2. 软著申请(CPCC,账号名私存) | 定名(已);材料生成(§6) | 待生成材料+线上提交;**建议加急** |
| 3. DNS + 隐私页挂网 | Cloudflare 自定义域 | **完成**:`https://saydo.octoooo.com/privacy` 200 |
| 4. App 备案(腾讯云新增 App 服务) | 定名(已)+域名 DNS(已)+鸿蒙叶证书(已) | **已提交初审**(订单号私存),腾讯云审核中。字段快照已移出公开稿,存根见 [tencent-icp-app-filing.md](2026-08-13-tencent-icp-app-filing.md) |
| 5. 三店建应用占名 | ASC 网页/Play Console/AGC | **完成**(店名「说到」);Bundle ID 已先行 |
| 6. 公安备案 | App 备案号下发后 30 日内 | 未到期 |
...
- **App 名称**:说到(zh)/ SayDo(en。App Store en 店名精确 `SayDo` 被占,提审前定后缀方案,如 `SayDo – Talk It Done`)
...
- **平台变体**:
  - App Store 中国区:描述末尾附 `ICP 备案号:【下发后填】`
...
## 5. App 备案(已提交;本节不再当填写表维护)

腾讯云「新增 App 服务」已交初审。提交原文、进度、三平台公钥/MD5、备注、观察项以 **[2026-08-13-tencent-icp-app-filing.md](2026-08-13-tencent-icp-app-filing.md)** 为准;证书抄 [filing-cheatsheet.md](filing-cheatsheet.md)。
...
3. **文档鉴别材料**:用户手册 30–60 页(截图+操作说明)。底稿=DEPLOY 清单+移动使用动线+桌面三面一栏说明,配真机截图。

注意:

- 开源不影响登记(著作权自动产生,登记仅为权属证明);
- 软著名称与商店名不必逐字一致,但含「说到」主词,商店审核关联更顺;
- **加急通道(数百–千元级,7–15 工作日)是整条流水线唯一花钱能显著提速的环节**——若赶国内鸿蒙/安卓首批上架,建议直接加急,关键路径从约两个月压到约两周。
