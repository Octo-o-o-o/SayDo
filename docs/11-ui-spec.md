...
> 设计基因(**2026-09-15 起 supersede**):起步继承 OctoBlog / OctoDesk 的 Soft Glass 体系(雾灰蓝画布、玻璃面板),2026-08-13 换芯为纸上账本(宣纸画布、实底纸面、松烟墨、矿彩语义色、印泥朱盖章);**2026-09-15 owner 决策单第 13 节裁定:White Edition 为产品 UI 新方向**,合同要点与迁移规则见 §0.2。纸上账本体系进入「迁移前状态」——既有账本实现继续有效并受本篇约束,新页面与新批次按 White 方向执行;White 版 token/组件规范由迁移批产出并回写本篇。玻璃遗产只剩 Dark 主题暂留的夜色,归「夜账本」二期(随 White 重定)。原 Soft Glass 与账本历史表述见 git 历史。

## 0. 设计原则(五条,冲突时按序让位)

1. **诚实优先**:UI 不得美化状态——没有假百分比、没有伪精确数字(unknown 显示"还没有确切数字")、状态词三级纪律(收到/等你验收/已交付)与 10 §1 一致;视觉强调服务于"需要人"的时刻,不服务于装饰。公开能力声明受 §10.3 上限约束,证据词义见 [06 §7](06-references.md)。
...
### 0.2 White Edition 方向(2026-09-15 owner 决策单第 13 节裁定)

> **定位**:White Edition 是产品 UI 的新视觉方向,不是仅对外演示资产。方向实物 = `demo/saydo-investor-demo-white-edition.html`(对外投资人原型,全 mock,自成合同);本节是方向合同,实施照抄仍需迁移批产出的 token/组件规范。

**White 视觉语言合同要点**(自方向实物提炼,迁移批依此推导 token):

1. **白底实面**:`#fff` 系表面 + 细分隔线,不用磨砂/玻璃/彩色画布;层级靠线与留白,不靠投影与色块。
2. **近黑主键与墨文**:主动作按钮与标题用近黑(`#111`–`#252525` 阶,方向实物 `--ink:#252525`),正文灰阶;**全页不依赖彩色语义块**——状态以文字、细线与单强调呈现(与 §0 原则 3/4 同向,White 把它推到无色化)。
3. **编辑部式排版**:大标题强层级、短行宽、克制留白;数字与英文可用等宽/表格字距点缀。
4. **线框/几何图形**:装饰性插图用线框几何(网格、轮廓),不用彩色插画与照片。
5. **单一主强调**:同屏一个主动作(近黑实心),其余动作降级为描边/文字钮。

**迁移规则**(防止半改与双视觉真相):

1. 未迁移页面继续按本篇账本规范维护,**不得局部混入 White 元素做成半成品**;新页面与重构页面按 White 方向执行。
2. §2 token 三层结构与现值在 White 迁移批产出新规范前**继续冻结有效**;迁移批按「改 token 走轻量评审回写本篇」同一纪律替换,并同步处理 §2.7a 写死色值门禁的 White 版口径。
3. Dark「夜账本」二期与移动 `--m-*` 字阶随 White 版规范一并重定,不单独先行。
4. §0 五原则与 §0.1 十原则是**功能合同**(诚实状态、单一强调、球权分流、账本语义、零 emoji),与视觉皮肤无关,White 迁移不削弱其中任何一条;状态词纪律仍归 docs/10。
5. 方向实物中的 mock 内容**不构成产品承诺**:103 个演示动作 ID 是覆盖辅助而非 API 清单;全局 `S` 状态、模拟计时、localStorage 身份、⌘K/导览等交互是演示机制,生产实现须走真实合同与既有组件;「离线可用·演示环境」类标识在演示物上必须保留。
...
M1 确认卡只呈现当前数据面可证明的三段:问题、durable `expiresAt` 倒计时、动作行。动作固定为“做”(副文字节点直接逐字渲染当前卡的 `prompt_text`,不加前后缀、不另造建议)、“不要 · 不按这个来”、“撤销 · 当我没问过”;过期文案是“过期自动搁置”。**按 kind 定文案(GAP-02 残项 2.1)**:卡正文 = `/api/attention` confirmation 条目的 `title`(即 daemon `prompt_text` 投影);按钮取自 `confirmKind`(与 `CONFIRM_KINDS` 同源,与桌面 Chat 共用 `lib/confirmCardCopy` 一个映射):`memory` 显示「记 / 不用记」、kicker「记忆 · 信息确认 · 不是授权」、原则句「过期即丢,不会偷偷记」;登记的其它 kind 沿用「做 / 不要」;表外或缺失 kind 回落「确认 / 不」,不把未知动作说成会自动执行。建议/代价/证据/锚点属 M1.5,不得填演示数据。`mobile_lan` 不具备 S2 配对身份,runtime approval 在手机裁决时如实提示回受信桌面处理,不得伪装成 voice 放行。stale 卡只说“这张卡已不在待办里(可能已在桌面处理,或已过期搁置)”并在存在真实 focus/lane 目标时给“去泳道看”,不得伪称已由桌面处理;missing 零动作。底部只提供菜单钮与文本输入条;话筒图标的唯一行为是聚焦输入,placeholder 固定“用键盘上的话筒说话”,不得实现假按住说话。连接呈现由 `connecting/online/offline` 状态机驱动,非 online 时只禁发送按钮,输入与键盘话筒仍可编辑草稿。M-Chat 直接复用分树外 VoiceProvider 的既有 `turn.text` 与 VoiceHub 广播,桌面与手机同开时回复在双端同显;M1 不新增跨 Brain/tool 的接纳 outbox 或回执协议,per-session 多端定向留 M2。
...
**可用供给列表**(只列可用项;列表准入≠公开声明四态,检测/`logged_in`/self-test 不得写成「支持」,见 [06 §7](06-references.md) 与 §10.3):
...
- 每行:名称 + 一句人话(「已登录 · 本机用过 2 个模型」/「已存 key · 计费以服务商为准」)。版本号/构建 hash 移入未就绪入口与设置页。人话不得写固定秒数、不得由登录态写成订阅已含或零额外费用。
- 排序沿用 `sortCliCapabilities`;首项默认选中。
- 空态(2026-09-15 修订):「登录一个本机 CLI,或添加 API 直连」一句之下**必须列出已接线各家 CLI 的可粘贴修复行**(来源 = probe 里 `found=false` 且 `provider` 非 null 的条目的 `auth.fixHint`,由 daemon `CLI_CATALOG.loginHint` 生成,console 不自行维护第二份命令表),每行「名称 + 修复行」,末尾提示「装好并登录后点『重新检测』」;+「添加 API 直连」动作(展开 baseURL/key/模型表单;提交后按 one-key 语义构造完整四槽 pending,进入下方确认流)+ 未就绪入口。理由:全新机器上未就绪折叠区为空,用户拿不到任何可执行命令(2026-09-15 首启实测)。
...
- 全 CLI 方案必须写明 dialog「每轮约 15-25 秒」(槽行内保留「CLI 慢速」观察提示,必须带「约」,不得写成保证;禁止「API 秒回」/固定秒数 SLA,见 §10.3),并让 owner 分别确认 evaluator 隔离边界与同族风险,不得静默代写 ack。失败配置不激活。
...
- 页尾:逃生门下方细墨线 + 两端小字:左「本机运行 · 外发范围以当前配置为准」;右「版本 `<sourceRevision` 前 7 位>」(取自 `/health` identity,失败不显示不报错)。禁止写「数据不出这台电脑」或全本地/不出设备总括承诺(§10.3)。
...
### 10.3 公开声明上限(PG-01A C0,2026-08-31)

> 本节是公开面(console 文案、README、官网、release、模板)的**声明上限**。四态与证据词义以 [06 §7](06-references.md) 为准;本批只收紧声明,不改 UI 布局、控制流或 09 运行时枚举。中英文公开面同一套四态与上限,不得中英各说各话,也不得在页脚/免责声明里写限制、在正文写相反承诺。

1. **四态文案上限**(每条声明绑定候选/路径/条件,缺一不得升档):
   - `supported` → 可写「支持」;
   - `conditional` → 必须同时写出条件,条件内可写「可用」,不得省略条件称支持;
   - `preview` → 可写「预览」;不得写支持、开箱即用、已通过普通用户验收;
   - `unsupported` → 可写「不支持」或不出现在默认选择器;不得写可用/支持。
2. **证据不得串档**:检测成功、`logged_in`、self-test 通过、真实端到端是四件不同事实(06 §7.2)。向导「可用供给列表」只是 inventory/admission,不是 `supported`。
3. **首批人群与语音**:目标首批包含开发者与普通用户,完整语音可选;这是产品目标,不是已验收事实。未经真人验收,不得写普通用户已开箱即用。文本使用与语音部署分开声明:文本入口可独立验证;语音按 §5.10 `VoiceTransport` 实际状态说明,未配置时保持「语音未配置(可选)」且键盘输入始终可用。不得因语音可选降低 Gate 0 / S3 / 授权 / 审计,不得删除现有语音能力,也不得把文本可用写成离线或全本地支持。
4. **供给与费用禁推**:CLI 登录不能推断订阅权益或零额外费用;「任一 CLI / 任意兼容 API 都可用」禁止无条件出现;OpenAI 兼容不能推断工具或 Structured Outputs 等价;模型响应身份未知仍拒绝,不得写成已接入。无权利/TCK/新鲜政策证据时,公开声明不得高于 `conditional` / `preview`,安全缺省 `unsupported`。
5. **时延与数据边界**:不承诺固定秒数 SLA。CLI oneshot 的「约 15-25 秒」只许作为典型观察提示(必须带「约」,不得写成保证)。禁止把 API 写成「秒级返回」。禁止「全本地」「所有数据不出设备」「数据不出这台电脑」总括承诺;外发范围只许指向当前配置(模型/语音/connector)。
6. **语料不是产品支持**:`LIVE` 条目只表示该题需要当前来源,不得写成 connector 已实现或 986 条来源已闭合。现役 986 source 本批是 unresolved requirement;600 题范围/标签不得为了声明通过而改写。connector、远程业务、团队版、移动正式能力未获对应授权前保持 `unsupported` 或 `preview`,不得借双人群目标升档。

C1 起,现役公开根上的文案必须能投影到上述四态之一;本节与 06 §7 冲突时以更紧的上限为准。公开根由 `scripts/check-active-claims.mjs` 扫描;console 只改 claim copy,不改布局或控制流。
