...
"对话感"的判定标准与实现无关:**流式低延迟**(P50 ≤ 1.5s 且 P90 ≤ 2.5s 同判;样本 ≥20 才判定,少样本或非有限/逆序时间戳记 `undeterminable` 不算 pass;取消/超时/缺段轮进分母,按文本轮/PTT/免手/工具轮分别出分布;`llm_first_token`/`tts_first_byte` 是响应到达/整句合成完成的近似,时钟为 daemon 收到事件的时刻——GAP-02 2.4)、**可打断**、**多轮澄清**。
