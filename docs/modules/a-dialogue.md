...
- **设计要点**:① 两类行为:准备知识(proactive)+ 回答问题(reactive);人格 = 简短、口语、不念代码;② 工具耗时 >2s 必接自然过渡语;③ 高危工具复述确认 + daemon 二次校验(03 §4);④ 对话档恒 API(BYOA 判死,07 D18 结案表);⑤ 状态词三级纪律与数字纪律由 golden 锁(10 §1)。
