...
- **设计要点**:① **凭据字面量检测先于 `classifyTrust` 与任何 `requestedTrust` 豁免**(AS-01;09 `memory_secret_literal`):`MemoryLedger.add` 在 classify/insert 之前扫描 claim 与将落盘 source 字符串,规则=`findCredentialLiteralSpans`(豁免仅完整覆盖凭据跨度才跳过,局部相交不豁免);命中不 insert,audit 只记 kind+spanDigest,日志/错误/TTS 不回显原文;`user_stated` 不能绕过;一条拒写只隔离该次 `add`,不停项目登记,也不得借中央 guard 已拒绝而回滚外层事务。直入 trusted 仅 `user_stated` 与 `auto_low_impact`,后者过**独立机械判定器**(source ∈ {git-tracked repo_file, user_edit} ∧ 事实性陈述;指令词/第三方一律降 candidate;不经 Brain——owner 2026-07-24);**M0 只收 user_stated/user_approved**(04 §1.4 安全边界:第三方内容永远不能定义偏好/权限/凭据);**模型工具入口不得自授 M0 信任(SD-2,09 §4)**:`remember` 的 `user_approved` 不可自报,tier=M0 走普通记忆确认环(confirm kind=memory,`memory/m0Confirm.ts` 消费),确认前账本零写入;**readiness 联动(A3-armed 2026-07-28,09 §13 covered 块)**:add/correct 事件可带 readinessKey(候选绑定;`user_approved` 不再是工具可自报值,只能由 confirmReadiness 确认环产生);claim 失效(invalidate/forget_soft/forget_hard/supersede/expiresAt——active 投影全枚举)⇒ 其支撑的 ReadinessBinding 非现役 ⇒ 对应就绪 key 回 unknown(撤销传导,派发事务权威复核);② **forget_hard 是 append-only 的唯一例外**:tombstone(target ids/digests+generation)+ 就地覆写历史 claim/source + memoryGeneration 同事务 +1;清除多目标(FTS 行/投影/摘要)重放幂等;**备份例外**:快照备份不逐条清,登记待过期,`backup_retention_days`(缺省 30)到期整份删,话术如实告知(10 #38);③ FTS 用普通表(非 external-content),删除用标准 `DELETE`;④ 人工编辑 knowledge/*.md → daemon 监测 diff → 生成 `correct(source.kind=user_edit)` 回账本(双向)。
...
- **设计要点**:① **预算化**:超限产 partial manifest + 进度话术(如实"还没读完",不装读过);**凭据命中不是 partial**(AS-01:首次 raw staging 前扫描组装后的五件落盘文档并整体拒绝新 generation,旧 pointer/status 不变;不写原始绝对 workspace)。**`.cursor/rules` 读取**(M7):仅直系 `*.md|*.mdc`,上限见 `KNOWLEDGE_PRIVACY_LIMITS`/`classifyRulesReadBound`(含 relativeSourceIdentityMaxChars=269 与 relativeSourceMaxChars=784;声明支持来源经 foundationRulesRelativeSource 必须可进 safeHits,不得在定位串留下反斜杠/控制字符(Unicode Cc 封闭集 U+0000..U+001F ∪ U+007F ∪ U+0080..U+009F)/凭据原文,不得以含 U+007F 的 identity 进 safeHits);超界与凭据命中一样在首次 staging 前失败,不是 KEY_FILES oversized skip,也不是 2000 字切片后扫描。② generation 原子切换(奠基升级不半生不熟);③ 预热用 git diff 种子(上次快照以来变更);④ 沉思档承载(BYOA 订阅主场,07 D18);⑤ Quick-奠基复用:应急车道 P1 的 P0 兜底(05 §6 盲区表态);⑥ **Git 私有 write set**(AS-02):create-only ignore + 本批 5 次保护查询(`rev-parse --is-inside-work-tree` + 两路径 `check-ignore -v` + 两路径 `ls-files`)验证实际保护;失败只停本次投影。
- **依赖**:B2/B4(写入)、B5(rg 现读)、E1(沉思档);被 A3/B1 消费。
- **失效与恢复**:中断 → partial manifest 可续跑;generation 未切换前旧底座继续可用。凭据/保护失败三类可区分(单条未保存 / 刷新失败仍用旧有效知识 / 首次暂无底座);修复源文件或 Git 保护后走既有奠基/重试真正发布新一代,不要求重装或关闭保护。
