...
| 能力 | 状态 | 备注 |
|---|---|---|
| 桌面服务(daemon + 控制台)· macOS / Windows / Linux | 现在可用 | 源码形态已经可运行;v0.1.0-rc.8 固定 URL 仅在 GitHub Release 出现且发布检查全绿后生效。npm registry / Homebrew 与桌面 App 壳仍规划中;常驻安装(launchd)与系统通知目前只有 macOS 实现 |
