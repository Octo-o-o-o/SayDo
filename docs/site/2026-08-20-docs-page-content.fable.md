...
- 文档口径以 macOS 桌面服务当前版本为准;移动 App 均未上架,手机相关内容以「可以这样连」而非「商店下载」为准(§13)。

## 1. 「说到」(SayDo)是什么

「说到」(SayDo)是**一个你能对着说话的高级助手**,运行在你自己的 Mac 上。你只管把事情聊清楚——甚至边聊边想清楚;它先吃透你的项目上下文,把每件事记成一本账;听懂了、火候够了,就主动提议开工,驱动你电脑上已有的 AI 工具去办;执行和检查都跑完,它主动叫你来验收;你点头,才算交付。
...
- **先吃透,再办事**:第一次接触一个项目,它先做一次「奠基」——扫清项目结构与关键文件、吸收仓里已有的约定、生成持久的项目知识底座;之后每次对话都带着对你项目的理解,而不是从零开始。这是「说到」和「新开一个聊天窗口」的分水岭。(当前版本的奠基是确定性的机械管道;由沉思档模型做的深度研究是进行中的下一档,§9.2)
...
- **不是云端代跑**。没有「说到」的服务器,没有账号注册。重活在你自己的电脑上跑,数据存在你自己的设备上;你接入的第三方 AI(订阅 CLI 或 API key)由你的电脑直接调用,开发者不经手、不可见。
...
| 概念 | 一句话 | 详见 |
|---|---|---|
| **沟通面 / 执行面** | 手机和浏览器是沟通面(说话、看进展、拍板、审批);你的 Mac 是执行面(奠基、记忆、驱动 AI、后台办事)。沟通天然是移动的,执行天然是固定的 | §3.1、§13 |
...
> 目标:一台 Mac、一个你已登录的 AI CLI(或一个 API key),十几分钟跑起来并开口聊。

### 4.1 前提

| 项 | 要求 | 说明 |
|---|---|---|
| 系统 | macOS(Apple Silicon / Intel 均可) | 常驻、通知、打开浏览器等链路都是 macOS 实现;其他系统未支持 |
...
诚实标注:**当前奠基是确定性的机械管道(清单 + 摘录 + 文档生成),不含由沉思档模型做的 LLM 深度研究**——后者是进行中的下一档。
...
| **云端级联语音**(更准更自然) | `.env` 填火山豆包三键 + 起 Python 语音管线 | ASR = 火山豆包 sauc 流式(带**热词偏置**:你纠正过的术语、项目符号自动注入,实测术语召回 +10 点);TTS = 豆包 seed-tts-2.0 v3 双向流式(首句先播、可随时打断);当前识别形态 = 按住说话松手后整段识别(实时字幕进行中) |
| **手机原生语音** | iOS / Android 壳 | 走系统语音识别与合成(设备端),不需要 Python 管线 |
...
- **tailnet(Tailscale)面**:`config.toml` `[t2].tailnet_hosts = [...]`(纯主机名 / IP 显式白名单,**禁通配**,含任一非法项整面不开)+ `[t2].listen`;`just t2-pair` 出配对 URL(一次性注入手机本地会话,深链之后不带令牌)。tailnet 面可看任务、批 S2 级审批(review / decide / 记忆候选批准),合并链动作与 `/dev/*` 注入通道 403 引导回桌面。
...
沟通天然是移动的,执行天然是固定的:手机只做沟通面(说话、看进展、拍板、审批封顶 S2),重活留在你的 Mac;手机断连不影响后端执行。远程通道默认只读;电话 DTMF(规划中)只做 ack / snooze / 拒绝,不做任何审批。
...
- **语音的数据去向(两端不同,请分清)**:手机端与浏览器系统语音在设备 / 浏览器侧识别;**桌面端配了火山豆包 key 时,音频经 WebSocket 发到火山引擎云端识别与合成**——这是你自己配置、你的电脑直连的第三方服务。缺省 `store_audio = false`(当前版本根本没有写录音文件的代码路径)、`store_transcript = true`(可关;关了轮内流程照常、只是不落盘,并如实标注)。
- **什么流量会出网**(只有两类,且都是你自己配置的上游):① 你接入的 AI 上游——CLI 订阅工具走它们各自的官方通道,API key 直连你配置的端点;② 语音识别与合成(火山豆包,仅在你启用桌面云端语音时)。可选的 ntfy 推送是第三类(你配置的 topic,标题经脱敏)。
...
| 能力 | 状态 | 备注 |
|---|---|---|
| macOS 桌面服务(daemon + 控制台) | 现在可用 | 源码分发;npm / Homebrew 包与桌面 App 壳规划中 |
...
**是把我的项目上传到云端代跑吗?** 不是。没有云端服务器;重活在你的电脑上跑;你接入的第三方 AI 由你的电脑直接调用。

**需要什么前提?** macOS + Node 22 + 一个已登录的 AI CLI(或一个 API key)。语音和执行都是可选增量(§4)。
...
**支持 Windows / Linux 吗?** 暂不支持,当前面向 macOS(常驻、通知、打开浏览器等链路为 macOS 实现);没有明确时间表,不承诺。
