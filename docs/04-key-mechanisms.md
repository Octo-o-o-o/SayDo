...
- **凭据写前拒绝(AS-01,与预算 partial 分轨)**:ledger `add`(含 supersedes)扫描将落盘的 claim/`source.quote`/`source.ref`(豁免一律完整覆盖语义:仅覆盖整段凭据跨度才跳过;PEM 内局部引用与只覆盖 token 末尾的合法 ID 均不豁免,独立 env/digest/ID/变量引用仍可过);foundation 在首次 mkdir staging 前扫描**组装后**将写入的 `core.md`/`inventory.md`/`build-test-run.md`/`conventions.md`/`manifest-gen-N.json`(来源包括 KEY_FILES excerpts、现场 `.cursor/rules`、package scripts、justfile 任务名、AGENTS/CLAUDE 摘录)。**`.cursor/rules` 实际读取**须先过 contracts `classifyRulesReadBound`(直系 `*.md|*.mdc`;opendir 有限循环,先 stat 再有限读;单文件 65536B / 16 个文件 / 32 dirent / 名称 255B / 名称缓冲 4096B / 合计 131072B;safeHits 相对来源须经 contracts `foundationRulesRelativeSource` 安全表示(普通名 identity 最长 269=len(.cursor/rules/)+NAME_MAX;非普通名 `.cursor/rules/_enc/`+UTF-8 百分号编码,最长 784=len(.cursor/rules/_enc/)+3*NAME_MAX;取消未登记 240;POSIX 合法 `a\b.md` 与含控制字符/凭据字面量的直系名必须可表示且不得把文件改名为另一个文件,不得在定位串留下反斜杠/控制字符(Unicode Cc 封闭集 U+0000..U+001F ∪ U+007F ∪ U+0080..U+009F,不含 U+0020/U+007E/U+00A0)/凭据原文;不得以含 U+007F 的 identity 进 safeHits));超界 `foundation_build_restricted`、不写 staging、旧 generation 不动。禁止 `readdirSync`/`readFileSync` 整目录整文件后再 `slice(0,2000)` 当读取上限;2000 字只是通过读取上限后的落盘切片。不把 KEY_FILES `FILE_SIZE_LIMIT` oversized skip 套到 rules,不新增全仓扫描。**消除**这些文档中的原始绝对 `workspace` 路径,不把路径当作凭据 kind;若实施仍写入 `this.workspace`,测试红。记忆命中 ⇒ 该条不 insert(`memory_secret_literal`);不得把外层事务整段回滚说成「其它工作继续」(reanchor 其它 claim 仍并入并归档,nominate 其余轮次继续,对话/项目登记继续)。foundation 命中 ⇒ 在首次 staging 写入前终止**本次新 generation**:不写 staging、不 publish、不改 `current.json`/旧 generation status,不得把旧 manifest 改成 `partial` 同时发布新版。预算超限的既有 `partial` 续跑合同不套用到凭据命中。已有已验证 generation 时界面必须说「未更新仍用旧底座」;首次无旧知识不得假称可保留(`foundation_build_restricted` + 三类失败,09)。
- **私有投影 Git 保护(AS-02)**:create-only ignore 覆盖 `.saydo/foundation/` 与 `.saydo/knowledge/`;非 Git 跳过 Git 查询仍做凭据检测并可写本地;worktree 的 `.git` 文件用 `git -C <workspace>` 查询;根外 symlink / 保护不足 / 写失败停本次私有投影(`git_protection_insufficient`),不自动改用户 Git。修复源文件或 ignore/untrack 后走既有奠基/重试可发布新一代。
...
4. **"要不要现在就开始?我按每步问你来跑。"**——现役拍板只开放**逐步确认**(`step_confirm`)。直达验收(`direct_to_review`,口语「一口气跑完」)保留底层设计与 schema,标 designed/deferred(PG-01B;D3 未签),**不再作为现役用户可选能力**;旧数据携带 direct 值 fail-closed,不能拍板。AI 仍可按任务风险给建议,但不得把直达档当作现役入口,也不得用二选一话术请用户切到直达。
...
| S1 写(worktree 内) | 改代码、跑**已登记的验证**(冻结 argv + 配置闭包)、本地 commit | 自动放行;**未登记的 package script / `<pm> exec <bin>` / 本地 `go run`·`cargo run` 等执行型入口按能力归 S2**(它们加载 agent 可写的脚本与 runner 配置,与 node/just 同档;SD-3 2026-09-08,不按测试工具名豁免) |
...
| 绕过 hooks 的 git 形态(GAP-02 2.2,AS-05 剩余 grammar) | `git commit --no-verify` / `git commit -n`(含 `-an`、`-nm msg` 簇)/ `git merge --no-verify` ⇒ 至少 S2(`install_dependency`,target `git-hooks-bypass`);`git push --no-verify` ⇒ S3(与 `-c core.hooksPath=` 同档 `delete_data`);`merge -n`(--no-stat)、`cherry-pick -n`(--no-commit)、`push -n`(--dry-run)不是绕过,取值参数后的 token 是值 | 按提级后的档位审批,只取较高者;登记 verify 冻结闭包不因提级解冻(gate 层 `matchesFrozenVerify` 仍先于本分类) |

**S3 语音绝不放行**(含复述短语):同一麦克风、同一 ASR 的复述**不构成独立认证因子**(录音/旁人/合成语音都绕得过,NIST SP 800-63B 也不允许声纹作认证);语音只能"导航到审批卡 + 朗读将发生什么 + 留痕用户在场"。合并动作属 S3,永远由人触发——**"人触发"的精确语义(R-A 2026-07-26 拍板 WebAuthn 形态后收窄)= 人在屏幕上完成一次本机强认证(WebAuthn platform authenticator;例如 macOS Touch ID/设备密码、Windows Hello 或 Linux passkey/安全密钥,`authStrength=os_biometric`)签发一张单次消费的 S3 收据,daemon 凭收据执行合并**(收据 = 09 §3.3 `S3MergeReceipt` 判别型,普通屏幕收据不构成,Codex 21 A1);这与"人自己去终端 `git merge`"是同一"人触发"语义的两种落地(前者 Tier1 缺省,后者 = 无 passkey 时的降级人工交接,§5 合并链见 09 §3.3/§6.1/§13)。daemon 无收据不得自发合并;WebAuthn 断言在 daemon 本地校验(challenge-response,防重放),不是"语音批 merge"的换皮。
...
- **远程通道**:手机/电话默认**只读**(听汇报)。**现役(PG-01B,`safe_default=remote_business_403`)**:`via="tailnet"` / `via="mobile_lan"` 的 HTTP 业务 `/api/**` 一律 403,远程 WS 业务连接 fail-closed,远程下发指令不是现役入口。历史口径「远程下发指令需已配对设备 + PIN/推送确认,且封顶 S2」为 designed/deferred(DF-REMOTE-REOPEN),不得当作现役入口;电话 DTMF 只做 ack/snooze/拒绝,**不做批准**(口述 PIN 不算强认证);S3 永远只走已认证屏幕。
...
### 5.4 执行模式:两档(直达验收 / 逐步确认;现役仅逐步确认)

> **现役口径(PG-01B,2026-09-04)**:当前只开放逐步确认。直达验收档 designed/deferred,D3 未签,不是现役用户可选能力。下文两档对照、预授权清单、升档重念与拍板二选一话术保留为设计合同与存储兼容,不得当作现役入口;旧 `direct_to_review` 值 fail-closed。
...
- **拍板 UI/话术纪律(防旁路)**:预选位**永远等于生效默认**(项目默认,出厂=逐步确认);AI 的推荐只以徽章 + 一句理由呈现,**不占预选位**。**现役(PG-01B)**不提供直达/逐步二选一,确认句只说明按逐步确认执行;历史中性二选一("一口气跑完,还是每步问你?")为 designed/deferred,不得当作现役主动话术。不得演化为"我推荐 X,好吗?"的是/否偏置;**用户不置可否时按逐步确认执行**。理由:生成计划的模型不能同时决定"要不要盯着它"的缺省答案(§2.3 相关错误链)。
- **Tier 映射**:Tier 1(交互审批型运行时,现行为 Claude Code / Cursor CLI hooks)两档共用 adapter,差异全在审批裁决策略,永不使用 bypassPermissions;Tier 2(预授权型运行时,**按运行时能力探测,不按型号**——含经 Hopper 的批式路径)直达验收 = 预授权清单编译成后端配置 + 沙箱兜底,逐步确认 = **步序循环**(计划切步、每步一个 dispatch、步末 settle→汇报→确认→续跑;Brain 如实说明"这个后端只能每步结束时问你,不能动作瞬间拦")。
...
| 模式 \ 后端 | Tier 1(CLI hooks 薄执行器,轻任务) | Hopper 批式(重任务,P0.5) |
|---|---|---|
| 逐步确认(出厂默认) | [ok] 步骤边界停靠 = 当前 run 在边界 settle,确认后以续跑协议恢复(09 §9);S2 逐个经 CLI hook 上浮 | [fail] **unsupported(首发)**:批式流水线无步序停靠点——拍板时如实说明,给两个显式选项:任务够轻改走 Tier 1,否则改直达验收;**步序循环形态(计划切步、每步一 dispatch)P1 再评**,动作级确认待 Hopper M3b |
...
- **命名纪律**:UI 与话术不用"全自动"(对外承诺过头——S3 与验收永远在人手里);AI 复述固定为"一口气跑到等你验收",不跟读用户口中的"做完";对非技术用户可说"一口气跑到等你确认"。逐步确认的推荐话术带一句"改代码、跑登记好的测试不会问你"(防被理解为每步都吵;未登记的临时测试/构建命令会问一次,SD-3)。
