...
function sampleManifest(overrides = {}) {
  return buildTrackedReleaseAssetManifest({
    ...
    buildId: `0.1.0-rc.13+${"a".repeat(12)}.test`,
    protocolVersion: "1.0.0",
    assets: [
      {
        filename: "saydo-cli-0.1.0-rc.13.tgz",
        entryCount: 4,
        contentDigest: sha("b")
      },
      { filename: "SHA256SUMS" },
...
function testRuleset() {
  ...
  evaluateActiveTagRuleset([qualifyingRuleset({ conditions: { ref_name: { include: ["refs/tags/v0.1.0-rc.13"], exclude: [] } } })], tag);
  if (!githubRefPatternMatches("refs/tags/v*", "refs/tags/v0.1.0-rc.13")) throw new Error("v* 应覆盖 rc.5");
  ...
  expectThrow("exclude tag", () =>
    evaluateActiveTagRuleset(
      [qualifyingRuleset({ conditions: { ref_name: { include: ["refs/tags/v*"], exclude: ["refs/tags/v0.1.0-rc.13"] } } })],
      tag
    )
  );
  ...
  expectThrow("~ALL unsupported", () => githubRefPatternMatches("~ALL", "refs/tags/v0.1.0-rc.13"));
  expectThrow("character class unsupported", () => githubRefPatternMatches("refs/tags/v[0-9]*", "refs/tags/v0.1.0-rc.13"));
  expectThrow("double star unsupported", () => githubRefPatternMatches("refs/tags/**", "refs/tags/v0.1.0-rc.13"));
...
function currentAvailabilityBody() {
  return "v0.1.0-rc.13 固定 URL 已由不可变 GitHub Release immutable v0.1.0-rc.13 GitHub Release https://saydo.octoooo.com";
}
...
async function testDeployStateMachine() {
  const sites = [
    { project: "saydo", directory: "deploy/saydo-octoooo-com", previewBranch: "preview-v0.1.0-rc.13", productionBranch: "main" },
    { project: "saydo-link", directory: "deploy/link-saydo-octoooo-com", previewBranch: "preview-v0.1.0-rc.13", productionBranch: "main" }
  ];
  const seed = { release: { tag: "v0.1.0-rc.13" }, publicMain: sha("1").slice(0, 40), publicCi: { workflowRunId: 9 } };
        ...
        try {
          const evidence = await runPagesDeployStateMachine({
            ...
            listDeployments: async ({ project, environment }) => {
              ...
              const branch = environment === "preview" ? "preview-v0.1.0-rc.13" : "main";
              return [
                deploymentRecord({
                  project,
                  environment,
  ...
  if (
    ...
    JSON.stringify([
      "preview:saydo:preview-v0.1.0-rc.13",
      "preview:saydo-link:preview-v0.1.0-rc.13",
...
`
  ...
  try {
    const extraReturned = await runPagesDeployStateMachine({
      evidenceSeed: {
        release: { tag: "v0.1.0-rc.13", unknownRelease: extraLeak },
        publicMain: seed.publicMain,
  ...
  expectStableReject("revoked release", () => projectDurableEvidence({ ...started, release: revokedProxy({ tag: "v0.1.0-rc.13" }) }), illegal);
  ...
  expectStableReject(
    "revoked assets",
    () => projectDurableEvidence({ ...started, release: { tag: "v0.1.0-rc.13", assets: [revokedProxy({ name: "x", size: 1 })] } }),
    illegal
  );
  expectStableReject(
...
async function testFinalIndependentReviewRegressions() {
    ...
    sites: [
      {
        ...
        planned: {
          previewBranch: "preview-v0.1.0-rc.13",
  ...
  const smSeed = { release: { tag: "v0.1.0-rc.13" }, publicMain, publicCi: { workflowRunId: 9 } };
