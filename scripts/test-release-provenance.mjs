...
function sampleManifest(overrides = {}) {
  return buildTrackedReleaseAssetManifest({
    ...
    buildId: `0.1.0-rc.11+${"a".repeat(12)}.test`,
    protocolVersion: "1.0.0",
    assets: [
      {
        filename: "saydo-cli-0.1.0-rc.11.tgz",
        entryCount: 4,
        contentDigest: sha("b")
      },
      { filename: "SHA256SUMS" },
...
async function testAssetManifest() {
  ...
  // v2 核心语义回归锚:实测记录里的机器相关字段(bytes/sha256/npmIntegrity 属包装层,
  // 随 node patch/平台漂移——2026-08-26 实测 7098 字节差全在 gzip 层)不参与跨机绑定;
  // 只要内容摘要与 entryCount 一致,必须通过。
  const machineVariant = [
    { ...manifest.assets[0], bytes: 999999, sha256: sha("e"), npmIntegrity: "sha512-other" },
    { ...manifest.assets[1], bytes: 80, sha256: sha("c") },
    { ...manifest.assets[2], bytes: 40, sha256: sha("d") }
  ];
  assertAssetsMatchTrackedManifest(machineVariant, manifest);
  // 内容层漂移必须拒绝:contentDigest 与 entryCount 是跨机绑定的主体。
  const contentDrift = structuredClone(manifest.assets);
  contentDrift[0] = { ...contentDrift[0], contentDigest: sha("f") };
  expectThrow("asset contentDigest mismatch", () => assertAssetsMatchTrackedManifest(contentDrift, manifest));
  const entryDrift = structuredClone(manifest.assets);
  entryDrift[0] = { ...entryDrift[0], entryCount: entryDrift[0].entryCount + 1 };
  expectThrow("asset entryCount mismatch", () => assertAssetsMatchTrackedManifest(entryDrift, manifest));
  ...
  const noDigest = JSON.parse(serializeReleaseAssetManifest(manifest));
  delete noDigest.assets[0].contentDigest;
  expectThrow("tgz missing contentDigest", () => parseTrackedReleaseAssetManifest(JSON.stringify(noDigest)));
  const machineFieldInTracked = JSON.parse(serializeReleaseAssetManifest(manifest));
  machineFieldInTracked.assets[1].sha256 = sha("c");
  expectThrow("tracked manifest 不得含机器相关字段", () => parseTrackedReleaseAssetManifest(JSON.stringify(machineFieldInTracked)));
...
function testRuleset() {
  ...
  evaluateActiveTagRuleset([qualifyingRuleset({ conditions: { ref_name: { include: ["refs/tags/v0.1.0-rc.11"], exclude: [] } } })], tag);
  if (!githubRefPatternMatches("refs/tags/v*", "refs/tags/v0.1.0-rc.11")) throw new Error("v* 应覆盖 rc.5");
  ...
  expectThrow("exclude tag", () =>
    evaluateActiveTagRuleset(
      [qualifyingRuleset({ conditions: { ref_name: { include: ["refs/tags/v*"], exclude: ["refs/tags/v0.1.0-rc.11"] } } })],
      tag
    )
  );
  ...
  expectThrow("~ALL unsupported", () => githubRefPatternMatches("~ALL", "refs/tags/v0.1.0-rc.11"));
  expectThrow("character class unsupported", () => githubRefPatternMatches("refs/tags/v[0-9]*", "refs/tags/v0.1.0-rc.11"));
  expectThrow("double star unsupported", () => githubRefPatternMatches("refs/tags/**", "refs/tags/v0.1.0-rc.11"));
...
function currentAvailabilityBody() {
  return "v0.1.0-rc.11 固定 URL 已由不可变 GitHub Release immutable v0.1.0-rc.11 GitHub Release https://saydo.octoooo.com";
}
...
async function testDeployStateMachine() {
  const sites = [
    { project: "saydo", directory: "deploy/saydo-octoooo-com", previewBranch: "preview-v0.1.0-rc.11", productionBranch: "main" },
    { project: "saydo-link", directory: "deploy/link-saydo-octoooo-com", previewBranch: "preview-v0.1.0-rc.11", productionBranch: "main" }
  ];
  const seed = { release: { tag: "v0.1.0-rc.11" }, publicMain: sha("1").slice(0, 40), publicCi: { workflowRunId: 9 } };
        ...
        try {
          const evidence = await runPagesDeployStateMachine({
            ...
            listDeployments: async ({ project, environment }) => {
              ...
              const branch = environment === "preview" ? "preview-v0.1.0-rc.11" : "main";
              return [
                deploymentRecord({
                  project,
                  environment,
  ...
  if (
    ...
    JSON.stringify([
      "preview:saydo:preview-v0.1.0-rc.11",
      "preview:saydo-link:preview-v0.1.0-rc.11",
...
`
  ...
  try {
    const extraReturned = await runPagesDeployStateMachine({
      evidenceSeed: {
        release: { tag: "v0.1.0-rc.11", unknownRelease: extraLeak },
        publicMain: seed.publicMain,
  ...
  expectStableReject("revoked release", () => projectDurableEvidence({ ...started, release: revokedProxy({ tag: "v0.1.0-rc.11" }) }), illegal);
  ...
  expectStableReject(
    "revoked assets",
    () => projectDurableEvidence({ ...started, release: { tag: "v0.1.0-rc.11", assets: [revokedProxy({ name: "x", size: 1 })] } }),
    illegal
  );
  expectStableReject(
  ...
  await runFail("wrong project_name", {
    listDeployments: async ({ project, environment }) => [
      deploymentRecord({
        project,
        environment,
        branch: environment === "preview" ? "preview-v0.1.0-rc.11" : "main",
        publicMain: seed.publicMain,
        url: previewUrl(project),
...
async function testFinalIndependentReviewRegressions() {
    ...
    sites: [
      {
        ...
        planned: {
          previewBranch: "preview-v0.1.0-rc.11",
  ...
  const smSeed = { release: { tag: "v0.1.0-rc.11" }, publicMain, publicCi: { workflowRunId: 9 } };
