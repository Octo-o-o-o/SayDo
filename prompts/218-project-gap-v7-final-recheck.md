# SayDo 项目缺口治理 v7 最终对抗复查（fresh 218）
...
以及 `scripts/week-audit.mjs`、`scripts/release-file-transaction.mjs` 和方案直接点名的源码。

208–212、214 是旧版 `[fail]` 快照。213、215、216、217 都只有 ignored 事件流、没有 report：
分别因版本取代、运行中语义回修、20 分钟硬时限、以及启动后发现 D17 错列不存在的 216 report 而停止；
不得把它们写成通过。请独立复核当前 bytes，尤其验证：

1. D17 从 semantic freeze、机械最终化、owner raw SHA、prospective index、I:path、I 三路复审、E、
   amend/恢复到 future full-E-OID ff-only 的链路；E 前是否重验 preexisting literal staged blobs；
...
3. PG-01A 的实际 generator projection scope、986 对象 Q0 report、path+RFC6901 identity、唯一 producer、
   I/tree/digest 绑定和 mutation 是否可施工且不会假绿；
